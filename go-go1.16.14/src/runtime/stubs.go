...
// 注释：getcallerpc返回其调用者的程序计数器（PC）。
// getcallersp returns the stack pointer (SP) of its caller's caller.
// 注释：getcallersp返回其调用者的调用者的堆栈指针（SP）。
// The implementation may be a compiler intrinsic; there is not necessarily code implementing this on every platform.
...
// 注释：对getcallerpc和getcallersp的调用必须在被询问的帧中完成。
//
// The result of getcallersp is correct at the time of the return,
// but it may be invalidated by any subsequent call to a function that might relocate the stack in order to grow or shrink it.
